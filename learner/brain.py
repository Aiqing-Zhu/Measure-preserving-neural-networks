...
class Brain:
    ...
    def run(self):
        ...
        loss_history = []
        if not os.path.isdir('./'+self.filename+'/model'): os.makedirs('./'+self.filename+'/model')
        for i in range(self.iterations + 1):
            ...
            if i % self.print_every == 0 or i == self.iterations:
                ...
                if torch.any(torch.isnan(loss)):
                    ...
                    return None                
                if self.save:                   
                    torch.save(self.net, self.filename+'/model/model{}.pkl'.format(i))
            ...
            np.savetxt(self.filename+'/loss.txt', loss_record)
    ...
    def restore(self):
        if self.loss_history is not None and self.save == True:
            ...
            self.best_model = torch.load(self.filename+'/model/model{}.pkl'.format(iteration))
        else:
